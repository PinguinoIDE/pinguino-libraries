...
#define TMR2000US_DELAY         ((ABSOLUTE_MAX_US - ABSOLUTE_MIN_US) / (ABSOLUTE_MAX_TS - ABSOLUTE_MIN_TS))
...
{
    u8 n, t, num;
    //volatile u8 d = 10;//TMR2000US_DELAY;
            ...
            for (t = ABSOLUTE_MIN_TS; t < ABSOLUTE_MAX_TS; t++)
            {
                ...
                for (n = 0; n < MAXNBSERVOS; n++)
                {
                    ...
                    if (Servo[num].Activated)
                        if (Servo[num].PulseWidth == t)
                            BitClear(LATB, num);
                }
                Delayus(TMR2000US_DELAY);
                //Delayus(d);
            }
