    ...
    * Return:       the temperature previously acquired
    ...
    u8 DS18B20ReadMeasure(u8 bus, u8 index, DS18x20_Temperature * t)
    {
        ...
        while (crc_flag == 0)
        {
            ...
            // Read scratchpad
            OneWireWriteByte(bus, READ_SCRATCHPAD);

            // Receive 8 bytes from the DS18B20
            for (c = 0; c < 8; c++) 
            {
                SCRATCHPAD[c] = OneWireReadByte(bus);
                DS18x20CRC(SCRATCHPAD[c]);
            }
            // Receive the 9th byte [CRC] from DS18B20
            SCRATCHPAD[8] = OneWireReadByte(bus);
            
            // Compare calculated CRC with the 9th byte received
            if (gDowCRC == SCRATCHPAD[8])
            {
                ...
                // Flag for good CRC: this will exit the while statement
                crc_flag = 1;
            }
    ...
    * Returns:      the temperature previously acquired
    ...
    u8 DS18S20ReadMeasure(u8 bus, u8 index, DS18x20_Temperature * t)
    {
        ...
        while (crc_flag == 0)
        {
            ...
            // Receive the 9th byte [CRC] from DS18S20
            SCRATCHPAD[8] = OneWireReadByte(bus);
...
/*  --------------------------------------------------------------------
    ...
    * Returns       true if OK, false if error
    ...
    u8 DS18x20Read(u8 bus, u8 index, DS18x20_Temperature * t)
    {
        ...
        gNumROMs = DS18x20Find(bus);
        #ifdef DS18X20DEBUG
        Serial_begin(9600);
        Serial_printf("%d sensor(s)\r\n", gNumROMs);
        #endif

        // Which type of sensor is it ?
        if (index == SKIPROM)
        {
            gFamily = DS18X20ROM[1][0];
            #ifdef DS18X20DEBUG
            Serial_printf("Fam. = 0x%X\r\n", gFamily);
            #endif
        }
        else
            gFamily = DS18X20ROM[index][0];
        ...
        while(!DS18x20ReadMeasure(bus, index, t));
...
/*  --------------------------------------------------------------------
    ...
    * Note2: DS18S20 devices doesn't have a config register 
    ...
    u8 DS18x20Configure(u8 bus, u8 index, u8 TL, u8 TH, u8 config)
    {
...
/*  --------------------------------------------------------------------
    ...
    * Arguments:    bus = pin number where the 1-wire bus is connected.
    ...
    u8 DS18x20ReadRom(u8 bus, u8 index, u8 *romcode)
    {
        ...
        */
            // Reads the ROM Code from the device
            for (i = 0; i < 8; i++)
            {
                romcode[i] = DS18X20ROM[index][i];
                //romcode[i] = OneWireReadByte(bus);
                //DS18x20CRC(romcode[i]);
            }
        /*
            romcode[8] = OneWireReadByte(bus);
            
            // Compare calculated CRC with the 9th byte received
            if (gDowCRC == romcode[8])
                crc_flag = 1;
...
/*  --------------------------------------------------------------------
    ...
    * Description:  reads and returns a byte of data from the device.
    ...
    Note : This command can only be used when there is only ONE slave on
    the bus. It allows the bus master to read the slave’s 64-bit ROM
    ...
    u8 DS18x20ReadFamilyCode(u8 bus, u8 index)
    {
        ...
        if (DS18x20ReadRom(bus, index, romcode))
        {
            #ifdef DS18X20DEBUG
            Serial_printf("Fam. = 0x%X\r\n", romcode[0]);
            #endif
...
/*  --------------------------------------------------------------------
    ...
    * Returns :     false or number of devices found
    ------------------------------------------------------------------*/
        ...
        do {
            ...
            else
            {
                ...
                {                   // byte n and reset mask
                    DS18x20CRC(ROM[n]);// accumulate the CRC
                    n++;
                    k++;
                }
            }
