 ...
 * small web-pages.
 * The TCP stack is therefore a SDP-TCP stack (single data packet TCP).
 *
...
#ifndef IPARPUDPTCP_C
#define IPARPUDPTCP_C

#include <ethernet/net.h>
#include <ethernet/enc28j60p.h>
#include <ethernet/ip_arp_udp_tcp.h>

static u8 wwwport=80;
static u8 macaddr[6];
static u8 ipaddr[4];
static s16 info_hdr_len=0;
static s16 info_data_len=0;
static u8 seqnum=0xa; // my initial tcp sequence number
...
void step_seq(u8 *buf,u16 rel_ack_num,u8 cp_seq)
{
    u8 i=4;
    u8 tseq;
...
/*void make_tcphead(u8 *buf,u16 rel_ack_num,u8 mss,u8 cp_seq)
{
        u8 i=0;
        u8 tseq;
        while(i<2){
...
void make_tcphead(u8 *buf, u16 rel_ack_num, u8 cp_seq)
{
    u8 i;
    // copy ports:
...
void make_echo_reply_from_request(u8 spi, u8 *buf,u16 len)
{
    ...
    //
    ENC28J60PacketSend(spi, len, buf);
}
...
void make_udp_reply_from_request(u8 spi, u8 *buf,char *data,u8 datalen,u16 port)
{
    ...
    ENC28J60PacketSend(spi, UDP_HEADER_LEN+IP_HEADER_LEN+ETH_HEADER_LEN+datalen,buf);
}
...
void make_tcp_synack_from_syn(u8 spi, u8 *buf)
{
    ...
    ENC28J60PacketSend(spi, IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+4+ETH_HEADER_LEN,buf);
}
...
{
    if (info_data_len)
        return((u16)TCP_SRC_PORT_H_P + info_hdr_len);
    else
        return(0);
}
...
//08/09/2014 replaced by make_tcp_ack_from_any(u8 *buf,s16 datlentoack,u8 addflags) - see below
void make_tcp_ack_from_any(u8 spi, u8 *buf)
{
        u16 j;
        make_eth(buf);
        // fill the header:
...
//                make_tcphead(buf,info_data_len,0,1); // no options
        ...
        ENC28J60PacketSend(spi, IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN,buf);
}*/
...
void make_tcp_ack_with_data_noflags(u8 spi, u8 *buf, u16 dlen)
{
    ...
    buf[TCP_CHECKSUM_H_P]=j>>8;
    buf[TCP_CHECKSUM_L_P]=j& 0xff;
    ENC28J60PacketSend(spi, IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlen+ETH_HEADER_LEN,buf);
}
...
{
    make_tcp_ack_from_any(spi, buf,info_data_len,0); // send ack for http get
    // fill the header:
    // This code requires that we send only one data packet
    // because we keep no state information. We must therefore set
    // the fin here:
    buf[TCP_FLAGS_P]=TCP_FLAGS_ACK_V|TCP_FLAGS_PUSH_V|TCP_FLAGS_FIN_V;
    make_tcp_ack_with_data_noflags(spi, buf, dlen); // send data
...
void tcp_client_send_packet(u8 spi, u8 *buf,u16 dest_port, u16 src_port, u8 flags, u8 max_segment_size, 
    u8 clear_seqack, u16 next_ack_num, u16 dlength, u8 *dest_mac, u8 *dest_ip)
{
    u8 i=0;
    u8 tseq;
    u16 ck;
    ...
    buf[TCP_DST_PORT_H_P]= (u8) ( (dest_port>>8) & 0xff);
    buf[TCP_DST_PORT_L_P]= (u8) (dest_port & 0xff);

    buf[TCP_SRC_PORT_H_P]= (u8) ( (src_port>>8) & 0xff);
    buf[TCP_SRC_PORT_L_P]= (u8) (src_port & 0xff);
    ...
    // add 4 for option mss:
    ENC28J60PacketSend(spi, IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlength+ETH_HEADER_LEN,buf);
}
...
u16 packetloop_icmp_tcp(u8 spi, u8 *buf, u16 plen)
{
    u16 dat_p;
    ...
    if (buf[IP_PROTO_P] == IP_PROTO_TCP_V && buf[TCP_DST_PORT_H_P] == 0 && buf[TCP_DST_PORT_L_P] == wwwport)
    {
        ...
        {
            make_tcp_synack_from_syn(spi, buf); // make_tcp_synack_from_syn does already send the syn,ack
            return(0);
        }
